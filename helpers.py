...
class FaceRecognizer:
    def __init__(self):
        ...
        # update newest_id
        if len(self.identified_faces) > 0:
            self.newest_id = max(self.identified_faces.keys())
        if len(self.unidentified_faces) > 0:
    ...
    def load_face_encodings(self):
        ...
        self.newest_id = max(list(unidentified_faces.keys())) if len(unidentified_faces) != 0 else -1
    ...
    def recognize_faces(self, frame):
        ...
        for i in range(len(frame_face_encodings)):  # for each face in the frame
            ...
            face_image = face_image[:, :, ::-1]  # convert from BGR to RGB (which CV2 understands) for saving to image folder
            threading.Thread(
                target=cv2.imwrite,
        ...
        frame_face_locations = [(int(a * 4), int(b * 4), int(c * 4), int(d * 4)) for (a, b, c, d) in frame_face_locations]  # scale up frame face locations back to original size
